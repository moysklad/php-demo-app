...
* Примеры работы с PHP-сессией: обмен `contextKey` на `contextNonce` и повторное использование активного контекста в iframe/виджетах
...
`contextKey` - opaque-token, который МойСклад передает в URL iframe/виджета при открытии страницы.
Приложение не разбирает его содержимое и не использует как постоянный идентификатор пользователя.

Последовательность работы:
* Хост-окно открывает `entry/iframe.php?contextKey=...` или `entry/widget-*.php?contextKey=...`.
* Приложение обращается к Vendor API и получает `uid`, `accountId`, ФИО и права пользователя.
* В PHP-сессии сохраняется один активный пользовательский контекст: `uid`, `accountId`, `fio`, `isAdmin`, `contextNonce`, `createdAt`, `expiresAt`.
* Исходный `contextKey` в сессии не хранится и больше не используется.
